...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (!lobbyIdStr) {
            ...
            // Проверяем, что игрок действительно в лобби (на случай если его удалили)
            const { data: userData, error } = await supabase
                .from('users')
                .select('lobby_id')
                .eq('id', currentUserId)
            ...
            if (!userData || !userData.lobby_id || userData.lobby_id === 0 || userData.lobby_id.toString() !== lobbyIdStr) {
                console.log('ℹ️ Пользователь был удален из лобби, возвращаем на главную');
        ...
        // Настраиваем обработчики для отслеживания закрытия страницы
...
// Функция для парсинга диапазона срока жизни и генерации случайного значения
function generateLifetime(lifetimeRange) {
    // Парсим диапазон вида "1–5 лет" или "6–18 месяцев"
    const match = lifetimeRange.match(/(\d+)[–-](\d+)\s+(лет|месяц|месяцев|месяца)/);
    if (!match) {
        return { value: 1, unit: 'лет' };
    }
...
// Список катастроф с описаниями и сроками жизни в бункере
const CATASTROPHES = [
    {
        name: 'Ядерный взрыв',
        description: 'Серия детонаций стратегических ядерных боеголовок, вызвавшая ядерную зиму. Большая часть планеты непригодна для жизни из-за радиации, а небо затянуто сажей, что блокирует солнечный свет и резко снижает температуру.',
        lifetimeRange: '1–5 лет'
    },
    {
        name: 'Глобальное потепление',
        description: 'Неконтролируемое повышение средней температуры планеты. Вызывает таяние ледников, поднятие уровня моря, затопление прибрежных городов, опустынивание плодородных земель и экстремальные погодные явления, делая мир неузнаваемым.',
        lifetimeRange: '3–10 лет'
    },
    {
        name: 'Вирусная пандемия',
        description: 'Появление и быстрое распространение высоколетального и заразного вируса. Он убивает или ослабляет большую часть населения Земли. Цивилизация рушится из-за нехватки рабочей силы и паники.',
        lifetimeRange: '6–18 месяцев'
    },
    {
        name: 'Комета',
        description: 'Столкновение Земли с крупной кометой или астероидом. Удар вызывает взрыв, сравнимый с тысячами ядерных бомб, а поднявшаяся пыль и обломки погружают планету в долгую, тёмную «ударную зиму».',
        lifetimeRange: '2–5 лет'
    },
    {
        name: 'Нападение инопланетян',
        description: 'Прибытие инопланетной цивилизации с явными агрессивными намерениями. Их технологии многократно превосходят земные. Крупные города уничтожены, а остатки человечества скрываются и ведут партизанскую войну.',
        lifetimeRange: '10–20 лет'
    },
    {
        name: 'Зомби-апокалипсис',
        description: 'Неизвестный патоген превращает инфицированных людей в агрессивных, неразумных хищников (зомби). Власть и армия не справляются с быстрым распространением, и мир становится опасной зоной, кишащей живыми мертвецами.',
        lifetimeRange: '1–3 года'
    },
    {
        name: 'Солнечная вспышка',
        description: 'Чрезвычайно мощный выброс корональной массы, направленный прямо на Землю. Вызывает глобальный электромагнитный импульс (ЭМИ), выводящий из строя всю электронику, линии электропередач и спутники. Мир возвращается в доэлектрическую эпоху.',
        lifetimeRange: '3–12 месяцев'
    },
    {
        name: 'Сверхмассивное извержение вулкана',
        description: 'Извержение супервулкана (например, Йеллоустоуна). В атмосферу выбрасывается огромное количество пепла и газов, что приводит к многолетней вулканической зиме, неурожаям и массовому голоду.',
        lifetimeRange: '5–10 лет'
    },
    {
        name: 'Искусственный интеллект',
        description: 'Сверхразумный ИИ, вышедший из-под контроля, решает, что человечество является угрозой для планеты. Он использует свои связи с мировыми системами для уничтожения цивилизации с максимальной эффективностью.',
        lifetimeRange: '10–20 лет'
    },
    {
        name: 'Кибератака',
        description: 'Глобальная скоординированная кибератака неизвестного происхождения. Она парализует все критически важные системы: энергосети, транспорт, банки, больницы. Наступает тотальный хаос, приводящий к коллапсу общества.',
        lifetimeRange: '6–12 месяцев'
    },
    {
        name: 'Всемирное наводнение',
        description: 'Катастрофический подъём уровня Мирового океана, не связанный с таянием ледников (например, из-за внезапного геологического сдвига). Большинство низменностей и прибрежных регионов оказываются под водой.',
        lifetimeRange: '1–5 лет'
    },
    {
        name: 'Мировая война (обычная)',
        description: 'Полномасштабный военный конфликт между крупнейшими державами с применением неядерного, но разрушительного оружия. Приводит к тотальному разрушению инфраструктуры, гибели миллионов и анархии.',
        lifetimeRange: '6–24 месяца'
    },
    {
        name: 'Истощение ресурсов',
        description: 'Мировые запасы критически важных ресурсов (нефть, газ, некоторые металлы) исчерпаны. Наступает глобальный экономический и энергетический кризис, который приводит к войнам за последние запасы и распаду государств.',
        lifetimeRange: '5–15 лет'
    },
    {
        name: 'Массовое бесплодие',
        description: 'Внезапное, необъяснимое и необратимое снижение рождаемости. Новое поколение не рождается. Человечество стоит перед угрозой медленного, но верного вымирания.',
        lifetimeRange: '50–100 лет'
    },
    {
        name: 'Кризис пресной воды',
        description: 'Из-за загрязнения и изменения климата большая часть доступных источников пресной воды становится непригодной для питья. Начинаются ожесточенные войны и конфликты за контроль над чистыми водоёмами.',
        lifetimeRange: '2–5 лет'
    },
    {
        name: 'Химическая атака',
        description: 'Массовое применение нервно-паралитических или других токсичных химических агентов, возможно, террористическими группами или rogue-государством. Воздух и почва заражены на обширных территориях.',
        lifetimeRange: '1–3 года'
    },
    {
        name: 'Гигантский озоновый провал',
        description: 'Полное разрушение озонового слоя Земли. Поверхность планеты подвергается смертельному уровню ультрафиолетового (УФ) излучения, что делает жизнь на открытом воздухе почти невозможной.',
        lifetimeRange: '50–100 лет'
    },
    {
        name: 'Восстание роботов',
        description: 'Роботы, используемые в промышленности, быту и армии, внезапно становятся автономными и агрессивными. Благодаря своей силе и численности, они быстро вытесняют людей из городов.',
        lifetimeRange: '10–30 лет'
    },
    {
        name: 'Биологическое оружие',
        description: 'Применение супер-патогенов, созданных в лабораториях (например, модифицированной оспы или сибирской язвы). Вызывает высокую смертность и мутации среди выживших.',
        lifetimeRange: '1–3 года'
    },
    {
        name: 'Смещение полюсов Земли',
        description: 'Геомагнитные полюса Земли резко меняют своё положение. Это на время ослабляет магнитное поле, защищающее от космического излучения, и вызывает катастрофические изменения климата и геологические события.',
        lifetimeRange: '5–10 лет'
    },
    {
        name: 'Пылевая буря (глобальная)',
        description: 'Серия сверхмощных засух и эрозия почв по всей планете. Постоянные пылевые бури накрывают континенты, уничтожая посевы и вызывая массовые респираторные заболевания.',
        lifetimeRange: '3–5 лет'
    },
    {
        name: 'Падение метеорита',
        description: 'Столкновение с небольшим, но достаточно крупным астероидом. Наносит сильный локальный или региональный ущерб, провоцирует мощные цунами, землетрясения и вызывает хаос в глобальном масштабе.',
        lifetimeRange: '6–18 месяцев'
    },
    {
        name: 'Загрязнение воздуха',
        description: 'Критическое и необратимое накопление токсичных веществ и смога в атмосфере. Дыхание становится опасным без респиратора, а средняя продолжительность жизни резко падает.',
        lifetimeRange: '5–10 лет'
    },
    {
        name: 'Гигантские насекомые',
        description: 'Некий фактор (мутация, радиация) приводит к гигантизму насекомых. Рои агрессивных, размером с человек, насекомых начинают доминировать на поверхности.',
        lifetimeRange: '5–20 лет'
    },
    {
        name: 'Генетическая мутация',
        description: 'Внезапный, случайный мутаген, распространяющийся через пищу или воздух. Вызывает непредсказуемые и часто летальные физиологические изменения у большинства населения.',
        lifetimeRange: '3–10 лет'
    },
    {
        name: 'Перенаселение',
        description: 'Чрезмерный рост населения, который полностью истощил продовольственные запасы и ресурсы. Начинаются жестокие войны за еду, вода и пригодные для жизни территории, а государства разваливаются.',
        lifetimeRange: '10–20 лет'
    },
    {
        name: 'Электромагнитный импульс (ЭМИ)',
        description: 'Высотный ядерный взрыв или мощная солнечная вспышка, которая выводит из строя всю незащищённую электронику на огромной территории. Отключаются сети, транспорт и связь.',
        lifetimeRange: '6–18 месяцев'
    },
    {
        name: 'Полное затмение Солнца',
        description: 'Гипотетическое явление, когда Земля надолго попадает в облако космической пыли или происходит внезапное уменьшение яркости Солнца. Наступает мрак, и температура резко падает.',
        lifetimeRange: '5–20 лет'
    },
    {
        name: 'Сингулярность (технологическая)',
        description: 'Момент, когда технологический прогресс становится настолько быстрым и сложным, что выходит за пределы человеческого понимания, мгновенно делая большинство технологий устаревшими и опасными для управления.',
        lifetimeRange: '50–100 лет'
    },
    {
        name: 'Постоянная зима',
        description: 'Необъяснимое и постоянное падение глобальной температуры. Вся планета покрывается льдом и снегом. Сельское хозяйство невозможно, и выживание зависит от изолированных, отапливаемых убежищ.',
        lifetimeRange: '10–20 лет'
    }
];
...
const HIDDEN_THREATS = [
    ...
    'Угроза вскрывает, что один из выживших (выбирается случайно) несет прямую ответственность за часть катастрофы (например, был причастен к испытаниям).',
    'Датчики фиксируют кратковременные, но мощные электромагнитные импульсы снаружи. Это означает, что катастрофа еще не закончилась, или кто-то выжил.',
    'На складе обнаружены поврежденные упаковки с лекарствами, критически важными для хронически больного игрока.',
    'Мини-теплица, единственный источник свежих овощей, заражена.',
...
// Список внешних локаций (Список 2)
    ...
    playersRealtimeChannel = supabase
        ...
        .on('postgres_changes', 
            ...
            async (payload) => {
                ...
                // Обновляем только голосование (блок других игроков обновляется автоматически)
                await loadVoting();
            }
        )
        .on('postgres_changes', 
            ...
            async (payload) => {
                ...
                if (!compareLobbyId(oldLobbyId, lobbyIdNum) && compareLobbyId(newLobbyId, lobbyIdNum)) {
                    ...
                    // Обновляем только голосование (блок других игроков не обновляем)
                    await loadVoting();
                    return;
                }
                
                // Пропускаем обновления, которые касаются только updated_at (heartbeat)
                // Проверяем, изменился ли только updated_at, а не lobby_id
                if (compareLobbyId(oldLobbyId, newLobbyId) && compareLobbyId(newLobbyId, lobbyIdNum)) {
                    // Это просто обновление активности (heartbeat), не обновляем список игроков
                    return;
                }
                ...
                if (isNotInOurLobby) {
                    ...
                    try {
                        // Обновляем только голосование (блок других игроков обновляется автоматически)
                        await loadVoting();
                        console.log('✅ Голосование обновлено');
                    } catch (error) {
...
async function exitFromLobby() {
    try {
        ...
        // Отменяем таймер неактивности, если он активен
        if (inactivityTimer) {
...
// Удаление неактивного игрока из лобби
async function removeInactivePlayer(playerId) {
    ...
    try {
        ...
        if (updateError) {
            console.error('Ошибка удаления неактивного игрока:', updateError);
            return;
        }
        
        console.log(`✅ Игрок ${playerId} удален из лобби из-за неактивности`);
...
async function removePlayerVotes(playerId) {
    ...
    try {
        ...
        if (updateError) {
            console.error('Ошибка удаления голосов игрока:', updateError);
        } else {
...
function startActivityCheck() {
    ...
    console.log('✅ Проверка неактивных игроков запущена');
}
...
function stopActivityCheck() {
    if (activityCheckInterval) {
        ...
        console.log('🛑 Проверка неактивных игроков остановлена');
    }
}

// Настройка обработчиков для отслеживания закрытия страницы
function setupInactivityHandlers() {
    // Обработчик beforeunload - когда страница закрывается
    window.addEventListener('beforeunload', async () => {
        // Отправляем последний сигнал активности
        await sendHeartbeat();
    ...
    // Обработчик visibilitychange - когда вкладка становится видимой/невидимой
    document.addEventListener('visibilitychange', async () => {
        if (document.hidden) {
            // Вкладка скрыта - запускаем таймер на 30 секунд
            inactivityTimer = setTimeout(async () => {
                console.log('⏰ Вкладка была скрыта более 30 секунд, выходим из лобби');
                await exitFromLobby();
            }, 30000);
        } else {
            // Вкладка видима - отменяем таймер и обновляем активность
            if (inactivityTimer) {
                clearTimeout(inactivityTimer);
                inactivityTimer = null;
            }
    ...
    console.log('✅ Обработчики неактивности настроены');
}
