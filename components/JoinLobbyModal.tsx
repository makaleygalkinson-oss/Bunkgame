...
interface JoinLobbyModalProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (data: { playerName: string; gameId: string }) => void
}

export default function JoinLobbyModal({ isOpen, onClose, onSubmit }: JoinLobbyModalProps) {
  ...
  useEffect(() => {
    if (!isOpen) {
      setFormData({ playerName: '', gameId: '' })
    }
  }, [isOpen])

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (formData.playerName && formData.gameId) {
      onSubmit(formData)
    }
  }
    ...
    <div className="modal active" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="bg-cyber-yellow text-cyber-black px-8 py-6 flex justify-between items-center border-b-2 border-cyber-black">
          <h2 className="font-orbitron font-black text-2xl uppercase tracking-wider m-0">ПРИСОЕДИНЕНИЕ К ЛОББИ</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-8">
          ...
          <div className="mb-6">
            ...
            <input
              ...
              maxLength={6}
              className="w-full px-4 py-3 bg-cyber-gray border-2 border-white/20 text-cyber-text font-roboto text-base transition-all outline-none focus:border-cyber-yellow focus:shadow-[0_0_10px_rgba(255,215,0,0.3)] focus:bg-cyber-dark uppercase"
            />
          </div>
